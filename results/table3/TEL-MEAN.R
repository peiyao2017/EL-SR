...
thresholds=c(118,116,111,79)

tel1_mean=function(m=100,d=2,mu1=rep(0.5,times=d),threshold=200){
  false=0
...
for(i in 1:length(thresholds)){
  ...
  a=foreach(j=1:1000, .combine="c")%dopar%{
    ...
    library(MASS)
    library(mvtnorm)
    tel1_mean(threshold=thresholds[i],m=size[i])
  }
  for(j in 1:1000){
    a3[j]=a[3*j]
  }
