setwd("/panfs/jay/groups/20/panwei/wan01299/collider_bias/paper/table 3/ARL1")
...
thresholds=c(62,57,51,46.6)

tael1_laplace=function(m=100,d=2,mu1=rep(0.5,times=d),threshold=200){
  false=0
...
for(i in 1:length(thresholds)){
  ...
  a=foreach(j=1:1000, .combine="c")%dopar%{
    ...
    library(MASS)
    library(mvtnorm)
    tael1_laplace(threshold=thresholds[i],m=size[i])
  }
  for(j in 1:1000){
    a3[j]=a[3*j]
  }
