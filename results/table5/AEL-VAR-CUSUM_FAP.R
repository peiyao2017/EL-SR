...
ael1_var=function(m=100,d=4,min0=-3,max0=3,min1=-3.5,max1=3.5,threshold=200){
  false=0
...
for(i in 1:length(thresholds)){
  ...
  a=foreach(j=1:1000, .combine="c")%dopar%{
    ...
    library(MASS)
    library(mvtnorm)
    ael1_var(threshold=thresholds[i],m=size[i])
  }
  for(j in 1:1000){
    a3[j]=a[3*j]
  }
...
outname="table5_AEL-MEAN and VAR-CUSUM.RData"
