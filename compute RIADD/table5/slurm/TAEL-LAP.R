...
thresholds=c(49.5,48.5 ,47.5,51.8,1080)
N=c(100,100,100,100,1000)
trunctions=c(1000,1000,1000,1000,3000)


tael1_laplace=function(m=100,d=4,threshold=200,min0=-3,max0=3,min1=-3.5,max1=3.5,v=0){
  if(v<=2+1){
...
tael_laplace=function(m=50,d=4,threshold=200,min0=-3,max0=3,min1=-3.5,max1=3.5,trunction=1000){
  x=matrix(0,nrow=m,ncol=d)
  for(i in 1:m){
    x[i,]=runif(n=d,min=min0,max=max0)
  }
...
for(i in 1:length(size)){
  c1=foreach(l=1:1000, .combine="c")%dopar%{
    ...
    library(MASS)
    library(mvtnorm)
    tael_laplace(threshold=thresholds[i],m=size[i],trunction=trunctions[i] )
  }
  p=numeric()
  sddelay1=numeric()
  b1=numeric()
  ...
  for(j in 1:length(vs)){
    ...
    repeat{
      a=foreach(l=1:1000, .combine="c")%dopar%{
        ...
        library(MASS)
        library(mvtnorm)
        tael1_laplace(threshold=thresholds[i],m=size[i],v=vs[j])
      }
      for(k in 1:1000){
        a11[k]=a[3*(k-1)+1]
      }
...
save(result,file="TAEL-LAP.RData")
