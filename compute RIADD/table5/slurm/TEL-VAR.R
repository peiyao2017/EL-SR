...
thresholds=c(120,227,340,345,2e+11)
...
for(i in 1:length(size)){
  c1=foreach(l=1:1000, .combine="c")%dopar%{
    ...
    library(MASS)
    library(mvtnorm)
    tel_var(threshold=thresholds[i],m=size[i],trunction=trunctions[i] )
  }
  b1=numeric()
  p=numeric()
  sddelay1=numeric()
  for(j in 1:length(vs)){
    ...
    repeat{
      a=foreach(l=1:1000, .combine="c")%dopar%{
        ...
        library(MASS)
        library(mvtnorm)
        tel1_var(threshold=thresholds[i],m=size[i],v=vs[j])
      }
      for(k in 1:1000){
        a11[k]=a[3*(k-1)+1]
      }
...
save(result,file="TEL-VAR.RData")
