tel_laplace=function(d=4,m=100,threshold=200,min0=-3,max0=3,min1=-3.5,max1=3.5,trunction=1000){
...
tel1_laplace=function(m=100,d=4,threshold=200,min0=-3,max0=3,min1=-3.5,max1=3.5,v=0){
  if(v<=3){
    x=matrix(0,nrow=m,ncol=d)
    for(i in 1:m){
      x[i,]=runif(n=d,min=min0,max=max0)
...
for(i in 1:length(size)){
  c1=foreach(l=1:1000, .combine="c")%dopar%{
    ...
    library(MASS)
    library(mvtnorm)
    tel_laplace(threshold=thresholds[i],m=size[i],trunction=trunctions[i] )
  }
  ...
  for(j in 1:length(vs)){
    ...
    repeat{
      a=foreach(l=1:1000, .combine="c")%dopar%{
        ...
        library(MASS)
        library(mvtnorm)
        tel1_laplace(threshold=thresholds[i],m=size[i],v=vs[j])
      }
      for(k in 1:1000){
        a11[k]=a[3*(k-1)+1]
      }
...
save(result,file="TEL-LAP.RData")
