ael1_var=function(m=100,d=4,min0=-3,max0=3,min1=-3.5,max1=3.5,threshold=200,v=0){
  if(v<=d+1){
...
ael_var=function(m=100,d=4,threshold=200,min0=-3,max0=3,min1=-3.5,max1=3.5,trunction=1000){
  x=matrix(0,nrow=m,ncol=d)
  for(i in 1:m){
    x[i,]=runif(n=d,min=min0,max=max0)
  }
...
thresholds=c(0.7,0.68,0.67,0.45,2)
...
for(i in 1:length(size)){
  c1=foreach(l=1:1000, .combine="c")%dopar%{
    ...
    library(MASS)
    library(mvtnorm)
    ael_var(threshold=thresholds[i],m=size[i],trunction=trunctions[i] )
  }
  p=numeric()
  sddelay1=numeric()
  b1=numeric()
  for(j in 1:length(vs)){
    ...
    repeat{
      a=foreach(l=1:1000, .combine="c")%dopar%{
        ...
        library(MASS)
        library(mvtnorm)
        ael1_var(threshold=thresholds[i],m=size[i],v=vs[j])
      }
      for(k in 1:1000){
        a11[k]=a[3*(k-1)+1]
      }
...
save(result,file="AEL-CUSUM-VAR.RData")
