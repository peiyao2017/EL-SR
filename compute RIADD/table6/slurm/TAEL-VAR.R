tael_mean=function(m=100,d=2,lambda0=4,lambda1=4.5,threshold=200,trunction=1000){
...
tael1_mean=function(m=100,d=2,lambda0=4,lambda1=4.5,threshold=200,v=0){
  if(v<=2*d+1){
...
for(i in 1:length(size)){
  c1=foreach(l=1:1000, .combine="c")%dopar%{
    ...
    library(MASS)
    library(mvtnorm)
    tael_mean(threshold=thresholds[i],m=size[i],trunction=trunctions[i] )
  }
  ...
  for(j in 1:length(vs)){
    ...
    repeat{
      a=foreach(l=1:1000, .combine="c")%dopar%{
        ...
        library(MASS)
        library(mvtnorm)
        tael1_mean(threshold=thresholds[i],m=size[i],v=vs[j])
      }
      for(k in 1:1000){
        a11[k]=a[3*(k-1)+1]
      }
