...
repeat{
  a=foreach(i=1:1000, .combine="c")%dopar%{
    library(el.convex)
    library(MASS)
    el1_var(threshold =1170000,m=20)
  }
...
repeat{
  a=foreach(i=1:1000, .combine="c")%dopar%{
    library(el.convex)
    library(MASS)
    el1_laplace(threshold =180 ,m=20)
  }
...
repeat{
  a=foreach(i=1:1000, .combine="c")%dopar%{
    library(el.convex)
    library(MASS)
    ael1_laplace(threshold =43,m=20)
  }
...
repeat{
  a=foreach(i=1:1000, .combine="c")%dopar%{
    library(el.convex)
    library(MASS)
    tel1_laplace(threshold =63.8,m=20)
  }
...
repeat{
  a=foreach(i=1:1000, .combine="c")%dopar%{
    library(el.convex)
    library(MASS)
    tael1_laplace(threshold =51.8,m=20)
  }
...
el1_laplace=function(m=100,d=4,threshold=2.1,min0=-3,max0=3,min1=-3.5,max1=3.5){
  false=0
  x=matrix(0,nrow=m,ncol=d)
  for(i in 1:m){
    x[i,]=runif(n=d,min=min0,max=max0)
